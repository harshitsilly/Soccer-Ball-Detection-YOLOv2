...
###################  INSTALLATION NOTE #######################
##############################################################
# python downloadImage.py custom_dataset\\Car_License_Plate_Detection.json custom_dataset/images/img custom_dataset/images/ann
...
def maybe_download(image_url, image_dir,count):
    """Download the image if not already exist, return the location path"""
    fileName = image_url.split("/")[-1]
    filePath = os.path.join(image_dir, str(count) + ".jpg")
...
def get_xml_for_bbx(bbx_label, bbx_data, width, height):
    ...
    xml = "<object>\n"
    xml = xml + "\t<name>" + "licensePlate" + "</name>\n"
    xml = xml + "\t<pose>Unspecified</pose>\n"
    xml = xml + "\t<truncated>1</truncated>\n"
...
def convert_to_PascalVOC(dataturks_labeled_item, image_dir, xml_out_dir,count):
    ...
    try:
        ...
        filePath = maybe_download(image_url, image_dir,count)
        ...
        xml = xml + "<filename>" + str(count) +".jpg" +"</filename>\n"
        xml = xml + "<path>custom_dataset/images/img/" + str(count) + ".jpg" +"</path>\n"
        ...
        #output to a file.
        xmlFilePath = os.path.join(xml_out_dir, str(count) + ".xml")
...
def main():
    ...
    success = 0
    for line in lines:
        status = convert_to_PascalVOC(line, image_download_dir, pascal_voc_xml_dir,count)
        if (status):
            success = success + 1
